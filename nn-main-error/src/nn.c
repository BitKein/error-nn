...
void train_CUDA(nn_t *nn, ds_t *ds, int epochs, int size_batch, double lr){
    ...
    for (n=0; n < epochs;n++) {
        ...
        for (x = 0; x < n_batches; x++) {
            for(min_batch = (x * size_batch); min_batch < ((x + 1) * size_batch); min_batch++){
                ...
                i = order[min_batch];
                forward_pass_CUDA(nn, &ds->inputs[i * ds->n_inputs], A, Z); 
                loss += back_prop_CUDA(nn, &ds->outputs[i * ds->n_outputs], A, Z, D, d);
            }
            
            update_CUDA(nn, D, d, lr, size_batch);
        }
...
void test_CUDA(nn_t *nn, ds_t *ds){
    ...
    for(i = 0; i < ds->n_samples; i++){
        ...
        forward_pass_test_CUDA(nn, &ds->inputs[i * ds->n_inputs], A);
    }
